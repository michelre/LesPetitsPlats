...
function filterByQuery(recipe){
  return (filterByName(recipe) || filterByDescription(recipe) || filterByIngredientQuery(recipe))
}
...
function filterByIngredientQuery(recipe) {
  ...
  return recipe.ingredients.filter((ingredient) => {
    return ingredient.ingredient.toLowerCase().includes(query.toLocaleLowerCase())
  }).length > 0
}
